...
def build_scen_data(scen_file, year, scenario,value_slider,value_slider2, peak_hour, unit, kommun):
    ...
    url_all_labels = "https://raw.githubusercontent.com/ClaudiaAda/SUES-Digit/main/All_labels_Claudia3.json"
    ...
    for label in list(info_data.keys()):
        ...
        if info_data[label]["target"] != "link":
            ...
            if (("T0 " + label) in scen_labels) or label in constant_variables:
                ...
                scen_data["node"]["percentage"].append(percentage_value)


            # Assign position in x-axis to nodes // y-axis will be assigned later
            ...
            # The order is actualised for each column
            """ if column != info_data[label]["column"]:
                column = info_data[label]["column"]
                y = 0
            
            scen_data["node"]["y"].append(y)
            y+=1   """
